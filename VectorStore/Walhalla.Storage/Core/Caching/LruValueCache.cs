...
/// <summary>
/// Thread-safe LRU value cache.  All mutations are serialised on an internal lock so that
/// concurrent readers (under the <see cref="AsyncReaderWriterLock"/> read lock) can safely
/// populate the cache without racing with writers or with each other.
/// </summary>
internal sealed class LruValueCache
{
    ...
    // Approximate overhead per cache entry: key array + value array + Node object + LinkedListNode
    // + Dictionary entry + GC headers ~ 72 bytes conservative estimate.
    ...
    /// <summary>
    /// Returns the cached byte array by reference, without copying.
    /// The caller must treat the returned array as read-only for the duration of the read lock.
    /// </summary>
    ...
    /// <summary>
    /// Stores <paramref name="value"/> in the cache without cloning it.
    /// Use only when <paramref name="value"/> is a freshly-allocated array that the cache
    /// will own exclusively (the caller must not modify it after calling SetWeak).
    /// The key is still copied for storage.
    /// </summary>
